...
from boot import credentials       # import a configuration file with network credentials
...
ssid = credentials['ssid']  # Getting Wi-Fi SSID from secrets module
pw = credentials['pw']      # Getting Wi-Fi password from secrets module
...
while True:
    try:
        ...
        try:
            response = response.replace('slider_value_base_servo', str(deg_base_servo))
            response = response.replace('slider_value_right_link_servo', str(deg_right_link_servo))
            response = response.replace('slider_value_left_link_servo', str(deg_left_link_servo))
            response = response.replace('slider_value_end_effector', str(deg_end_effector))

        except Exception as e:
            response = response.replace('slider_value_base_servo', '0')
            response = response.replace('slider_value_right_link_servo', '0')
            response = response.replace('slider_value_left_link_servo', '0')
